...
| REL-02 | Default concurrency of three shares one worktree without recoverable checkpoints | **Fixed.** Two halves. The default global concurrency cap is now **1** across all six authority sites (feature 098), so a fresh install no longer lands unasked in a configuration it did not choose; the range stays `[1, 20]` and concurrent execution stays supported. The residual — that a checkpoint could not be attributed to one run in a shared worktree — is closed by FR-R3-004, and the framing in this row was wrong: the fix needed a source of attribution, not per-run isolation, and one already existed. Each phase's audit record declares the files it wrote, the patch is scoped to that declaration, and the whole-tree diff is re-read at every phase boundary to check the declarations account for what is present. Three runs in flight now yield three disjoint patches. Ambiguity still declines, with four bounded reasons in place of the blanket `concurrent-runs-share-one-worktree`, which nothing emits. See [`checkpoint-attribution-decision.md`](../architecture/checkpoint-attribution-decision.md). The shared worktree itself is unchanged and remains the operator's to reconcile. |
...
| Doc Drift (residual) | The 11 links the errata note left standing, and the fact that `docs:check` could not have caught any of them | **Fixed.** The 11 are closed the way the errata note describes them — pointers into components deleted by the operations task-first redesign — so each is now inline code plus a dated **Reading note** naming the commit that removed the target and stating what still exists. They are deliberately not repointed: rewriting a closed `DONE_*` report to match today's components would falsify the record rather than repair it. The tooling half is the more durable half. [`scripts/check-doc-links.mjs`](../../scripts/check-doc-links.mjs) now resolves every relative link in the curated markdown and `docs:check` runs it, so this class of drift fails a gate instead of waiting for a review. It reports 1,420 links across 237 files with none broken. Path existence only: `#anchor` and `#L42` fragments are stripped, because line anchors move with every edit and a gate that failed on them would be noise. The **semantic** drift half of the `docs:check` row below stays open — no gate reads a sentence and decides whether it is still true. |
...
| FUNC-01 / CTO-003 | Validated RunPlan inputs, supplemental context, and output targets never reach the backend prompt | **Fixed.** `ExecutionEnvelope` ([`src/contracts/run-request.ts`](../../src/contracts/run-request.ts)) is now the runtime contract, constructed only by `validateRunRequest()` and consumed **by reference** from the factory through the driver and phase runner to `prompt-builder.ts`, which renders bound inputs, supplemental context, declared output targets, and operator instructions as four request sections after Schegent's own output contract. `resolveRunOutputs` probes `run.envelope.outputs` instead of re-reading the queue row. Model/runner policy was already inside the envelope — `snapshotPhaseDef()` freezes each phase's effective runner, side-effects, evidence policy and prompt version at validation — and the mutation plan is a pure projection of that frozen array, with the approval receipt bound to its fingerprint; those three are documented rather than moved, because a persisted copy of a derived value is the second-source-of-truth shape the repo forbids elsewhere. `tests/lint/no-envelope-reconstruction.test.ts` fails the build on any second construction site. |
| FUNC-02, REL-01, CTO-007 | Mandatory queue identity, fenced ownership, coordinated reset | **Open — filed as round-3 backlog items.** Each is an architectural change spanning several seams, not a contained fix. |
...
workspace root: FR-R3-001 (FUNC-01, **landed**), FR-R3-002 (FUNC-02),
FR-R3-003 (REL-01), FR-R3-004 (REL-02 residual), FR-R3-005 (SEC-01 residual),
FR-R3-006 (CTO-007).
