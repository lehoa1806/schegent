...
- **Clear Done:** Removes `completed` items from the recent queue (fires `CMD_CLEAR_COMPLETED`). Disabled when no completed items exist.
- **Clean:** Removes both `completed` and `failed` items from the recent queue (fires `CMD_CLEAR_COMPLETED` + `CMD_CLEAR_FAILED`). Disabled when no terminal items exist. `cancelled` items can be removed individually from each row's action set.
