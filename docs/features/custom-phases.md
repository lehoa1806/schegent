...
| `evidencePolicy` | Optional `required`, `best-effort`, or `none`; omission resolves to `required`. **A declaration, not enforcement:** no host decision reads this value today, so all three settings classify a Phase identically. Declare it to state what the Phase owes; do not rely on it to withhold advancement. <!-- Source: src/contracts/process-definitions.ts --><!-- Source: src/config/process-catalog.ts --> |
