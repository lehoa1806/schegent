...
| `evidencePolicy` | Optional `required`, `best-effort`, or `none`; omission resolves to `required`. Controls **how a missing audit block is reported**, not whether the Phase advances: `required` warns as a constitution violation (the historical behaviour), `best-effort` records an expectation not met, `none` expects no block and says nothing. All three advance identically — use it to silence noise from a Phase that legitimately emits no block, not to withhold advancement. <!-- Source: src/contracts/process-definitions.ts --><!-- Source: src/parser/stdout-parser.ts --> |
