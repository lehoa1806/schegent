...
- **Direct VS Code commands** have the individual guards documented below, and one gate above all of them: every one of the 31 registrations goes through `registerGuardedCommand`, which classifies the ID from `src/contracts/entry-point-dispositions.ts`. The 24 `mutating` IDs are wrapped in a re-read of `vscode.workspace.isTrusted` that runs on **every invocation** — not a boolean captured at registration, because a command outlives its registration and can be invoked after trust is granted — and a refusal warns the operator, logs once at info, and returns `undefined` rather than throwing. The 7 `read-only` IDs are registered unwrapped, so reads pay nothing. An ID in neither map does not type-check at the call site and throws at registration if cast. Until FR-R3-136 (2026-08-28) this line read "none of the 27 registrations checks `vscode.workspace.isTrusted` directly", and it was accurate: the sidebar router was the only gate, so the palette, a task, and any other installed extension reached every mutation.
...
These 23 commands appear in the Command Palette. A question mark means that the field or whole argument is optional.
...
| `schegent.gitApprovals` — Schegent: Git Approvals | none | Lists the durable Git plan approvals stored for this workspace — pipeline, Git-capable phases, mutation-plan fingerprint, and when each was granted — and withdraws the one picked, or all of them, behind a modal confirmation. This is the observe-and-revoke surface FR-012 requires; the record lives in VS Code workspace state, which is not a file an operator can read or edit. Picking a row selects; only the modal's affirmative action withdraws. Classified `mutating` in `entry-point-dispositions.ts`, so an untrusted workspace refuses it at the point of effect; no primacy check. | <!-- Source: package.json --><!-- Source: src/commands/git-approvals.ts --><!-- Source: src/state/git-plan-grants.ts --> |
